...
public class ThongKeDao {
    ...
    String[] month = new String[12];
    try {
        PreparedStatement prep = MyConnect.conn.prepareStatement(
                "SELECT Month(ngaylap), Sum(tongtien) AS 'Doanh thu' " +
        ...
        while (rs.next()) {
            int monthIndex = rs.getInt(1);
            int revenue = rs.getInt(2);
            month[monthIndex - 1] = String.valueOf(revenue);
        }
...
public String[] tongChiThang(int nam) {
    ...
    try {
        PreparedStatement prep = MyConnect.conn.prepareStatement(
                "SELECT Month(ngaynhap), Sum(tongtien) AS 'Chi phi' " +
        ...
        while (rs.next()) {
            int monthIndex = rs.getInt(1);
            int expenses = rs.getInt(2);
            month[monthIndex - 1] = String.valueOf(expenses);
        }
