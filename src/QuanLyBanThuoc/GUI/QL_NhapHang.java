...
import QuanLyBanThuoc.BUS.CTPhieuNhapBUS;
import QuanLyBanThuoc.BUS.DangNhapBUS;
import QuanLyBanThuoc.BUS.NhanVienBUS;
import QuanLyBanThuoc.BUS.PhieuNhapBUS;
...
import QuanLyBanThuoc.DTO.KhachHang;
import QuanLyBanThuoc.DTO.NhanVien;
import QuanLyBanThuoc.DTO.PhieuNhap;
...
//	}
        ...
        if(txt_NCC.getText().equals("")) {
        	...
        	return;
		    	...
		    	String maNCC[] = txt_NCC.getText().split(" - ");
                        // Lấy mã phiếu nhập cuối cùng từ cơ sở dữ liệu và tăng thêm 1
                        int lastMaPN = pnBUS.getLastID();
                        int maPN = lastMaPN + 1;
		    	pnBUS.themPhieuNhap(maNCC[0], maNV[0], tongTien,maPN);
		    	//Nhập hàng
		    	nhapHang();
    ...
    Vector<Vector<Object>> rows = new Vector<>(); // Tạo Vector để chứa các dòng cần thêm vào bảng
    ...
    for (SanPham sp : dssp) {
        if (String.valueOf(sp.getMaSP()).equalsIgnoreCase(text) || sp.getTenSP().contains(text) || String.valueOf(sp.getSoLuong()).equalsIgnoreCase(text)) {
            ...
            row.add(sp.getTenSP());
            row.add(sp.getSoLuong());
            rows.add(row); // Thêm dòng dữ liệu vào Vector
        }
    }

    model_KhoHang.addRow(rows); // Thêm toàn bộ dữ liệu vào model
    tbl_KhoHang.setModel(model_KhoHang); // Cập nhật bảng
}
	...
	private void chonSanPhamNhap() {
        ...
        // Kiểm tra thông tin sản phẩm trước khi thêm vào bảng
	...
	private void xoaSPNhap() {
        ...
        JOptionPane.showMessageDialog(null, "Vui lòng chọn sản phẩm để xóa!", "Lỗi", JOptionPane.ERROR_MESSAGE);
    }
