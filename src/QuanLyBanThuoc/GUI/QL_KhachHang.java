	...
	public QL_KhachHang() {
		...
		btn_tim.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// Kiểm tra xem ô chi tiêu từ và đến có giá trị không
                              if (txt_tu.getText().trim().isEmpty() || txt_den.getText().trim().isEmpty()) {
            // Nếu một trong hai ô không có giá trị, thông báo lỗi
                                 JOptionPane.showMessageDialog(panel, "Vui lòng nhập giá trị đủ!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                          } else {
            // Nếu cả hai ô đều có giá trị, thực hiện tìm kiếm
                           timKiemChiTieu();
        }
			}
		});
		...
		btn_sua.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// Kiểm tra xem có hàng nào được chọn trong bảng không
                               if (tbl_KH.getSelectedRow() == -1) {
                              // Nếu không có hàng nào được chọn, hiển thị thông báo lỗi
                              JOptionPane.showMessageDialog(panel, "Vui lòng chọn khách hàng cần sửa trong bảng!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                             } else {
                              // Nếu có hàng được chọn, thực hiện hàm sửaKH()
                              suaKH();
        }
...
//		panel_2.add(btn_xuatExcell);
    ...
    int tongchitieu = 0; // Khởi tạo giá trị mặc định
    try {
        tongchitieu = Integer.parseInt(txt_tongchitieu.getText());
