...
import QuanLyBanThuoc.BUS.LoaiSanPhamBUS;
import QuanLyBanThuoc.BUS.SanPhamBUS;
import QuanLyBanThuoc.BUS.TCBUS;
import QuanLyBanThuoc.DAO.MyConnect;
import QuanLyBanThuoc.DTO.LoaiSanPham;
import QuanLyBanThuoc.DTO.SanPham;
import QuanLyBanThuoc.DTO.TacGia;
import QuanLyBanThuoc.GUI.MainFormGUI;
...
//	}
    ...
    private void addControls() {
		...
		lbl_SanPham.setForeground(Color.red);
		...
		panel.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)), "Thông tin sản phẩm", TitledBorder.LEADING, TitledBorder.TOP, null, Color.BLACK));
		...
		panel_tg.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)), "Thông tin thuốc", TitledBorder.LEADING, TitledBorder.TOP, null, Color.BLACK));
		panel_tg.setBounds(10, 203, 597, 183);//62
		//contentPane.add(panel);
		panel_tg.setLayout(null);
		
		JLabel lbl_MaTG1 = new JLabel("Mã Thuốc :");
		...
		JLabel lbl_TenTG = new JLabel("Xuất xứ :");
		...
		panel_tg2.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)), "Nơi xuất xứ", TitledBorder.LEADING, TitledBorder.TOP, null, Color.BLACK));
		...
		//-----------------Dialog quan ly xuat xu va loai sp------------------
		...
		panel_2.add(lbl_DonGia);
                ...
                JLabel lbl_MaTG = new JLabel("Xuất xứ :");
		...
		spi_SoLuong.setModel(new SpinnerNumberModel((int)1,(int) 1, null, (int)1));
		...
		panel_2.add(btn_ClearSP);
                ...
                ql_loaiSP_Button=new JButton("Quản lý loại SP và XX");
		...
		table_SP.setModel(new DefaultTableModel(
			...
			new String[] {
				"Mã sản phẩm", "Tên SP", "Số lượng", "Đơn giá", "Loại", "Hình ảnh","Xuất xứ", "Mô tả"
			}
		));
    ...
    private void select_cbx_find(){
        switch(cbx_find.getSelectedIndex()){
                        ...
                        case 1 ->{
                            lb_nhap_find.setText("Nhập Mã TC: ");
                        ...
                        case 2 ->{
                            lb_nhap_find.setText("Nhập tên sp: ");
                            txt_nhap_find.setEnabled(true);
                            btn_findSP.setEnabled(true);
                            txt_giatien1.setEnabled(false);
                            txt_giatien2.setEnabled(false);
    ...
    private void themSP() {
        // Kiểm tra nếu fileAnhSP là null
    if (fileAnhSP == null) {
        JOptionPane.showMessageDialog(null, "Vui lòng chọn ảnh sản phẩm!");
        return;
    }
    	String anh = fileAnhSP.getName();
         int selectedIndex1 = cbx_loai.getSelectedIndex();
    int selectedIndex2 = cbx_tg.getSelectedIndex();

    // Kiểm tra các combo box nếu không được chọn
    if (selectedIndex1 == 0 || selectedIndex2 == 0 ||
        txt_MaSP.getText().isEmpty() || txt_TenSP.getText().isEmpty() || 
        txt_DonGia.getText().isEmpty()) {
        JOptionPane.showMessageDialog(null, "Vui lòng chọn giá trị trong tất cả các ô!");
        return; // Dừng quá trình thêm sản phẩm nếu có trường thông tin bị trống
    }
        String comboBox1 = "" + cbx_loai.getItemAt(cbx_loai.getSelectedIndex());
        String comboBox2 = "" + cbx_tg.getItemAt(cbx_tg.getSelectedIndex()); 
       // Kiểm tra các trường thông tin nếu bị trống
    ...
    private void locSPTheoLoai() {
		...
		for( SanPham sp : spBUS.getSanPhamTheoLoai(maLoai[0])) {
    ...
    private void locSPTheoTG() {
    	...
    	spBUS.docListSanPham();
		model_SP.setRowCount(0);
		for( SanPham sp : spBUS.getSanPhamTheoTG(maTG[0])) {
		        Vector row = new Vector();
		        row.add(sp.getMaSP());
		        row.add(sp.getTenSP());		      
		        row.add(sp.getSoLuong());		       
